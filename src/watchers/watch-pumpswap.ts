...
const PROGRAM_ID = new PublicKey("pAMMBay6oceH9fJKBRHGP5D4bD4sWpmSwMn52FMfXEA"); // PumpSwap
const POOLS = new Set(ENV.PUMPSWAP_POOLS);

const filename = "pumpswap.jsonl";
...
console.log(
  `[pumpswap] watching program ${PROGRAM_ID.toBase58()}, commitment=${commitment}`
);
if (POOLS.size)
  console.log(`[pumpswap] filtering pools: ${[...POOLS].join(", ")}`);
console.log(`[pumpswap] writing logs → ${writer.path}`);
