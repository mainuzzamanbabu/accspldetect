...
const PROGRAM_ID = new PublicKey("LBUZKhRxPF3XUpBCjp4YzTKgLccjZhTSDM9YuVaPwxo"); // Meteora DLMM
const POOLS = new Set(ENV.METEORA_DLMM_POOLS);

const filename = "meteora-dlmm.jsonl";
...
console.log(
  `[meteora:dlmm] watching program ${PROGRAM_ID.toBase58()}, commitment=${commitment}`
);
if (POOLS.size)
  console.log(`[meteora:dlmm] filtering pools: ${[...POOLS].join(", ")}`);
console.log(`[meteora:dlmm] writing logs → ${writer.path}`);
